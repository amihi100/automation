...
public class twoDrivers {
	...
	static String pangoURL, panguGUID, carNumber, screenShotPath, filename, insuredName, insuredLastName,
			insuredIdDriver, insuredBirthDate, insuredLicenseIssueYear, city, street, houseNumber, email, cellphone,
			driver1Name, cardNumber, driver1LastName, driver1Id, driver1BirthDate, cardPersonalId,
			driver1LicenseIssueYear, cardFullName, expMonth, expYear;
	...
	// In case we want to use for next day start.
	...
	public static void defineVariables() throws Exception {
		try {
			...
			insuredName = "מוחמד";
			insuredLastName = "ג'עברי";
			insuredIdDriver = "326683455";
			insuredBirthDate = "1990";
			insuredLicenseIssueYear = "2009";
			city = "מג'דל שמס";
			street = "מג'דל שמס";
			houseNumber = "4";
			...
			// Driver1 details:
			driver1Name = "אבו יוסוף";
			driver1LastName = "אלעוברה";
			driver1Id = "330653783";
			driver1BirthDate = "1996";
			driver1LicenseIssueYear ="2014";
					//
					// CreditGaurd details
					cardFullName = "מוחמד ג'עברי";
			cardPersonalId = "890108558";
			cardNumber = "5326120384572410";
			expMonth = "09";
			expYear = "20";
			// Define URL.
			// DEV Environment: https://dev.pango-ins.co.il/api/test/url
			// STAGE Environment: https://stage.pango-ins.co.il/api/test/url
	...
	public static void initialOffer() throws Exception {
		try {
			...
			/*
			 * {val: 18, label: "17-20"} {val: 22, label: "21-23"} {val: 25, label: "24-26"}
			 * {val: 28, label: "27-29"} {val: 32, label: "30-34"} {val: 37, label: "35-39"}
			 * {val: 43, label: "40-46"} {val: 48, label: "47-49"} {val: 55, label: "50-59"}
			 * {val: 62, label: "60-64"} {val: 67, label: "65-69"} {val: 72, label: "70-74"}
			 * {val: 77, label: "+75"}
			 */
			TimeUnit.SECONDS.sleep(1);
			js.executeScript("window.ageRangeApo.__changeComponentValue('0', parseInt(22))");
			TimeUnit.SECONDS.sleep(1);
	...
	public static void finalOffer() throws Exception {
		...
		try {
			...
			// Screen second driver details 2.4.5
			wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("step-6-birthDate-3")));
			TimeUnit.SECONDS.sleep(1);

			driver.findElementByXPath("//*[@id=\"step-6-firstName-0\"]").sendKeys(driver1Name);
			driver.findElementByXPath("//*[@id=\"step-6-lastName-1\"]").sendKeys(driver1LastName);
			driver.findElementByXPath("//*[@id=\"step-6-personalId-2\"]").sendKeys(driver1Id);
			// birthDate
			TimeUnit.SECONDS.sleep(1);
			// stepTwoBirthPicker
			 JavascriptExecutor js2 = (JavascriptExecutor) driver;
			js2.executeScript(
					"window.stepTwoBirthPicker.setSelectedDate(new Date(" + driver1BirthDate + ", 1, 0, 0, 0, 0, 0))");
			TimeUnit.SECONDS.sleep(1);
			driver.findElementByXPath("//*[@id=\"step-2-birthDate-3\"]").click();
			TimeUnit.SECONDS.sleep(1);
			driver.findElementByXPath("//*[@id=\"ap-button-set\"]").click();
			// licenseIssueYear
			TimeUnit.SECONDS.sleep(1);
			js.executeScript("window.stepTwoLicensePicker.setSelectedDate(new Date(" + driver1LicenseIssueYear
					+ ", 6, 14, 0, 0, 0, 0))");
			TimeUnit.SECONDS.sleep(1);
			driver.findElementByXPath("//*[@id=\"step-2-licenseIssueYear-4\"]").click();
			TimeUnit.SECONDS.sleep(1);
			...
			// Remove comme when dev fixes update to stage
			// driver.findElementByXPath(
			// "/html/body/app-root/external-app-offer/aw-wizard/div/aw-wizard-step[9]/div/ul/li[3]/question-wrapper/div/div/single-checkbox/label/span")
			// .click();
			TimeUnit.SECONDS.sleep(waitBeforeClick);
