...
public class redPathFirstOfferPoliceRestricts {
	...
	public static void defineVariables() throws Exception {
		try {
			...
			pangoURL = "https://stage.pango-ins.co.il/api/test/url?ln=" + carNumber;
	...
	public static void initialOffer() throws Exception {
		try {
			...
			wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("step-0-q4-radio-2")));
			TimeUnit.SECONDS.sleep(waitBeforeClick);
			...
			driver.findElementByXPath(
					...
					.click();
			;
			// Screen1.9: Restricts in 3 years.
			wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("step-0-q7.2-0-radio-2")));
			TimeUnit.SECONDS.sleep(waitBeforeClick);
			driver.findElementByXPath(
		...
		try {
			...
			if (driver.findElementByXPath("//*[@id=\"modal-red-path\"]/div[1]/div/div[2]/div/div/div[2]/a[1]")
					...
					|| driver.findElementByXPath("//*[@id=\"modal-red-path\"]/div[1]/div/div[2]/div/div/div[2]/a[1]")
							.isDisplayed()) {
				System.out.println("You are in RED PATH - You have police restricts");
			}
